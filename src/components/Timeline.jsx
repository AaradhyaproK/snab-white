...
export default function Timeline() {
  const { data: steps } = useCollection("timeline", DEFAULT_STEPS, "phase", "asc");
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Map progress to steps index (0 to length-1) to determine which is active
    ...
    return (
      <section className="timeline-section" id="process">
        ...
        <div className="process-cards-grid" style={{ padding: '0 24px' }}>
          {steps.map((step, idx) => (
            <div className="process-card" key={step.id || idx}>
              ...
              <ul className="process-card-list">
                {step.bullets && step.bullets.map((bullet, bIdx) => (
